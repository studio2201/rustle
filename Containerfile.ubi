# =============================================================================
# rustle — Alpine container image (Unraid-compatible)
# =============================================================================
...
#
# Build (Podman):
#   cd ~/projects/studio2201/rustle
#   podman build -f Containerfile.ubi -t rustle:latest .
#
# Run (Unraid-style):
#   podman run -d --name rustle \
#     -p <host-port>:4502 \
#     -v /mnt/user/appdata/rustle:/config \
#     -e RUSTLE_PIN=<your-pin> \
#     rustle:latest
#

# Compare size:
#   podman images | grep -E 'rustle|studio2201/rustle'
# =============================================================================
...
# Install Tailwind CSS (rustle pins v3.4.17 for stability; v4 has breaking changes).
...
LABEL org.opencontainers.image.title="rustle" \
      org.opencontainers.image.description="rustle (Alpine, built on Alpine; Unraid-compatible)" \
      org.opencontainers.image.source="https://github.com/studio2201/rustle" \
...
COPY --from=builder /src/target/release/sh /usr/local/bin/rustle
RUN chmod +x /usr/local/bin/rustle
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD wget -qO- "http://127.0.0.1:${PORT:-4502}/health" >/dev/null || exit 1
