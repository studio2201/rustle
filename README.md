...
|---|---|---|
| `PORT` | The port number the backend HTTP server will bind to. | `4502` |
| `RUSTLE_PIN` | Optional PIN to restrict access to the application interface. | (None) |
| `RUSTLE_DATA_DIR` / `DATA_DIR` | Path to the directory where application data and database files are stored. | `data` / `/config` |
| `RUSTLE_ALLOWED_ORIGINS` / `ALLOWED_ORIGINS` | Comma-separated list of allowed CORS origins, or `*` to allow all. | `*` |
...
```bash
docker run -d -p 4502:4502 ghcr.io/studio2201/rustle:latest
```
